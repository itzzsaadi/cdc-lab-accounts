...
**This is a real gap, not an implementation detail to guess at.** Building the actual mobile/tablet layout for 11 screens — especially the day×party income grid (SCR-04), the densest and hardest screen to adapt — from prose alone risks inventing a visual decision the client hasn't approved. Recommend requesting mobile/tablet Stitch captures for at least SCR-02 through SCR-04 (the Operator screens, which are the ones actually required to work well on a phone per NFR-USE-07) before Phase 3 begins.
...
| -------- | -------------------------------------------------------------------------------------------------------------------------- |
| Operator | Home, Daily Expenses, Party Income, Counter Income (quick action), Cash Receipt (quick action), Sync Center                |
| Partner  | Everything Operator can, plus Monthly Expenses, Asset Register, Monthly Summary, Dashboard, Investment Statement           |
| Admin    | Everything Partner can, plus Administration Area (Users, Parties, Expense Items, Vendors, Profit Split, Historical Import) |

**What the Stitch HTML actually shows:** every one of the 10 authenticated screens (all except Sign In) renders the _identical_ sidebar — Dashboard, Asset Inventory, Financial Ledger, Reports, User Management, Settings, Logout — with no visible role-based differences anywhere in the markup. See the Critical finding in §25. The navigation _styling_ (icon-first compact mode, active-tab left-border accent, hover states, footer Settings/Logout grouping) is exactly what should be kept; the _link set shown to each role_ must be built per the table above, not copied from any single HTML file as-is.

## 9. Shared application shell

Common to every authenticated screen in the design:

- **Sidebar** (`src/components/layout`, future): 260px fixed desktop width, dark navy background (`on-background` token, `#123047`-family), brand mark + "LabFinance Pro" wordmark + tagline, nav links with icon + label, active-tab indicated by a 2px primary-teal left border, footer-pinned Settings/Logout group. Two documented sidebar variants exist in the handoff: a "labeled" variant (SCR-02, SCR-06/07/08/09/10) and a "compact-with-inline-label" variant (SCR-03/05/11) — both render the same link set; DESIGN.md additionally describes a fully icon-only compact mode for small screens, not captured in any screenshot.
...
- **Funding-source toggle** (Business/Partner radio pair with conditional partner selector) — Daily Expenses; the same pattern (as a checkbox, not a radio) appears in Asset Register's Cash-mode fields, though inconsistently required — see §25.
...
**Discrepancy found:** `DESIGN.md` documents a `rounded` scale of `sm: 0.125rem, DEFAULT: 0.25rem, md: 0.375rem, lg: 0.5rem, xl: 0.75rem, full: 9999px`. The `borderRadius` actually embedded in every one of the 11 HTML files' `tailwind.config` is different and consistent across all of them: `DEFAULT: 0.125rem, lg: 0.25rem, xl: 0.5rem, full: 0.75rem` (no `sm`/`md` keys at all, and `full` is **not** a pill/circle radius). Since the HTML is what was actually built and rendered in the screenshots, treat its values as the real ones for implementation — but `full: 0.75rem` is very likely a generation error (a "full" radius that isn't full/circular), not an intentional design choice. **Flag to the client for confirmation before Phase 3** rather than guessing which is correct.
...
- **Images**: exactly one placeholder "Administrator Profile" avatar photo per screen (9 of 11 screens), all externally hosted, AI-generated, licensing unconfirmed — recommend a component-based avatar (initials or uploaded photo) instead of stock imagery.
...
**Per the task's governing rule: the SRS controls every item below unless the client approves a requirements change. None of these has been resolved by this review — they are documented, not decided.**

### Critical

1. **Every authenticated screen's sidebar is identical regardless of role**, showing Asset Inventory, Financial Ledger, Reports, and User Management links to the Operator-facing Home and Daily Expenses screens exactly as it does to Partner/Admin screens. This directly conflicts with FR-AUTH-04 ("An Operator shall not be able to reach any screen or figure showing profit, loss, partner investment or the profit split, whether through the interface...") and SRS §2.6 ("Cannot do: See profit or loss, partner investment, settings or user management"). **Recommendation: the SRS's role-based navigation model controls.** Section 8 above specifies what each role's sidebar must actually contain; the Stitch sidebar's _visual styling_ is fine to keep, its _link set per role_ is not.
2. **Asset Register's Cash-acquisition mode makes the purchasing partner optional** (an unchecked "Funded by Partner" checkbox), but SRS DR-08/FR-AST-06 requires every Cash-mode asset to name a purchasing partner — there is no SRS concept of a cash purchase with no partner tag (only Instalment assets are untagged, per BR-09). **Recommendation: the SRS controls** — when Acquisition Mode = Cash, the partner field must be required, not optional; the checkbox pattern should become "mode = Cash always requires selecting a partner," not an independent toggle.
3. **The Offline Sync Center's worked conflict-resolution example is about an asset's "Calibration Status" and "Next Due Date"** — fields that don't exist in the SRS's asset schema (§6) and a concept (equipment calibration/maintenance tracking) the SRS never defines. Assets also carry no `client_uuid` and are not listed among FR-OFF-02's offline-enterable entities (daily expense, party income, counter income, cash receipt). **Recommendation: the SRS controls** — the two-column keep-local/keep-server _pattern_ is sound and reusable, but real conflicts will only ever involve the four financial entry types' actual fields (amount, date, item, funding source, etc.), never asset maintenance data. Do not build calibration/maintenance tracking as a side effect of reusing this screen's example.

### Important

4. `DESIGN.md`'s documented `rounded.full: 9999px` doesn't match the `0.75rem` actually built into every HTML file's `borderRadius.full` — see §12. Needs client confirmation, not a guess.
5. The connection-state/pending-upload-count indicator required "on every screen" by FR-OFF-03 is inconsistently present — see §18.
6. No date-range control is visible on the Monthly Summary Report despite FR-RES-02/03 requiring one (default current month, adjustable, one-action prev/next).
7. The product is branded "LabFinance Pro" throughout every screen's header (with "CDC Laboratories" appearing only on the Sign In screen). The SRS never assigns the system a marketing name — confirm with the client whether "LabFinance Pro" is the approved product name or a Stitch-generated placeholder to be replaced.

### Minor

8. Administration Area's "Historical Import" is shown only as a small upload widget, not FR-IMP-02's required "preview with errors marked" screen — likely just an unrendered tab state rather than a real gap, but worth confirming.
9. No negative-figure (loss) display convention (parentheses, sign, color) is shown anywhere, despite FR-RES-08 requiring the split to "apply the same split to a loss."
...
4. **Do not source the placeholder avatar photos.** They are externally hosted, licensing-unconfirmed, and likely ephemeral (see `docs/ui/ASSET_INVENTORY.md`). Build a real avatar component (initials or uploaded photo) instead.
5. **Treat every role-based navigation/permission cue in the HTML as illustrative, not authoritative.** The sidebar link set shown must be rebuilt per §8's SRS-derived table, and — per `CLAUDE.md` §15/§16 — every route must independently verify role server-side regardless of what any client-side navigation shows or hides.
6. **The `data-alt` attributes on avatar `<img>` tags are AI image-generation prompts, not accessible alt text** — some images additionally have a real (if generic) `alt="Administrator Profile"`, but this should be revisited for whatever real avatar solution replaces the placeholder photos.
7. **`href="#"` anchors throughout are non-functional placeholders**, standard for a static mockup — every one becomes a real route, form submission, or button `onClick` during implementation; none should be carried over as literal dead links.
8. **Money must still never touch a JS `number`.** Nothing in the Stitch HTML performs financial arithmetic (it's static markup with hardcoded example figures), so there is no calculation logic to accidentally inherit — but when wiring real figures into these layouts, `CLAUDE.md` §9/§10's `Decimal`-only rule applies exactly as it would anywhere else in the codebase.
9. **This design review changed no financial formula, permission rule, validation rule, or acceptance criterion.** Every business-logic requirement remains exactly as `docs/SRS.md` and `docs/REQUIREMENTS_TRACEABILITY.md` already state it; this document only adds the visual/interaction layer on top.
