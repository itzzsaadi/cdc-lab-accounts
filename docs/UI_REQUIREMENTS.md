...
Authoritative implementation guide for the frontend, derived from `docs/SRS.md` (functional requirements, business rules, permissions) and the Google Stitch UI/UX handoff (visual hierarchy, layout, color, typography, spacing, component appearance, responsive presentation, interaction presentation). This document remains the authoritative **reference** for every visual/interaction decision; it is no longer describing purely unbuilt work, though — Phase 3A has implemented the shared shell and reusable component foundation this document specifies. See the status note immediately below for exactly what now exists versus what a later phase (3B onward) still builds.

## Phase 3A implementation status (added after the shell was built)

The shared authenticated shell and reusable UI foundation this document specifies in §9–§12 are now implemented — see `docs/adr/0004-phase-3a-shared-shell.md` for the full design record and `docs/PROJECT_PLAN.md`'s Phase 3A section for the deliverable list. In this document's own terms:

- **§8 (navigation structure by role)** — implemented, but **incrementally**: only Home/Dashboard/Users exist as real routes today (per the approved Phase 3A decision), not the full target table below. Each later phase adds its own screen's nav entry when that screen is actually built, rather than this table being pre-populated against routes that don't exist yet.
- **§9 (shared application shell)** — implemented: sidebar, header (mobile menu control, contextual title, initials avatar, role label, user menu with sign-out — deliberately **no** notification bell or sync icon; see the Phase 3A decision on this), content frame, card primitive.
- **§10 (reusable component inventory)** — implemented at a minimal, non-speculative level: `Button`, `TextInput`, `Select`, `Checkbox`, `Card`, `Table`, `Badge`, `Alert`, `Modal` (native `<dialog>`), `EmptyState`, `LoadingSkeleton`, `Avatar`. Business-specific variants (the funding-source toggle, acquisition-mode toggle, sync/status badges with real states, the asset "Add New Asset" slide-over) remain unbuilt — those are Phase 3B/4/6 concerns layered on top of these generic primitives, not built ahead of the screens that need them.
- **§11 (typography)/§12 (color tokens)** — implemented in full in `src/app/globals.css`, sourced from the HTML files' embedded `tailwind.config` per this document's own resolution of the `DESIGN.md` vs. HTML radius discrepancy.
- **§13 (spacing)** — implemented (`sidebar-width`, `header-height`, `gutter`, `container-margin`, `touch-target-min`).
- **§18 (empty/loading/validation/permission/offline/error states)** — partially implemented: empty, loading, generic error, and permission-denied (`/forbidden`) now have a real, token-consistent treatment at the shell level; **validation** (field-level) and **offline/provisional** states remain unbuilt (no business form exists yet to validate; no offline queue exists until Phase 6).
- **§22 (icons, images, fonts)** — resolved: Inter and Material Symbols Outlined are both self-hosted via `next/font/local` from vendored, OFL-1.1-licensed files (never a live Google Fonts request) — see `public/design-assets/fonts/PROVENANCE.md`. The initials-based `Avatar` component is built and used; no external avatar photo exists anywhere in the app.
- **Everything else in this document** (§2–§7, §14–§17, §19–§21, §23–§27) remains exactly what it already was: a reference for Phase 3B and later phases to build the actual business screens from — none of that screen-specific content was implemented in Phase 3A, which deliberately built only the shell and foundation, not business functionality.
