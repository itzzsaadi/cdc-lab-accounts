...
### 13. Closure fix: FR-AUD-06 date flagging for `asset`/`capital_contribution`, via batched live lookup, never N+1

The original Phase 5 pass left `isEntryOverOneMonthOld` as `null` for every `asset` and `capital_contribution` audit row, because `src/server/mutations/{assets,capital-contributions}.ts` never write a business-date field (`acquiredOn`/`entryDate`) into the audit JSON snapshot at all — only `name`/`acquisitionMode` for assets, nothing date-related for capital contributions. **Fixed at closure**, not left as a permanent limitation: `resolveLiveBusinessYearMonths` (`src/server/queries/audit-log.ts`) resolves these two entity types' business date from the **live row** instead of the snapshot — a single batched `findMany` per entity type per page (never one query per audit row), keyed by the distinct `entityId`s actually present on that page/entity-history call. This is safe specifically because CLAUDE.md §11's physical-delete-rejection trigger guarantees the row always still exists for every `CREATE`/`UPDATE`/`ARCHIVE` audit entry on it. A malformed/non-UUID `entityId` (never produced by real mutations) is filtered out before the query runs, rather than allowed to throw a database-level UUID-cast error. `null` is preserved as the honest result for the one remaining case where it's genuinely unknown — an `INSTALMENT` asset whose `acquiredOn` was never set.

### 14. Closure: FR-RPT-05 — income by party across a chosen range

The initial Phase 5 pass disclosed FR-RPT-05 as a gap rather than building it, since it overlaps FR-PINC-08's own "arbitrary range" gap (ADR-0006) and both were deferred together. **Built at closure**: `getPartyIncomeReport` (`src/server/queries/party-income.ts`) reuses the exact aggregation shape `getPartyMonthlyTotals` already established (daily + monthly + cash-receipt components, combined total, archived-but-historical party visibility, zero-income party never omitted) but generalized to an arbitrary validated `{from, to}` range instead of one calendar month — Postgres-side `groupBy`/`SUM`, never a Node-side `reduce`, matching every other Phase 5 report total's NFR-PERF-04/06 discipline. `MONTHLY` rows are month-anchored, so the range's `MONTHLY` component sums every whole month the range touches (identical to `computeMonthlyResultTotals`'s own rule), while `DAILY`/`CASH_DIRECT` use the literal day range. Surfaced as its own screen, `/party-income-report` ("Income by Party"), gated `report:financial-summary` (Partner-minimum, the same permission every other Dashboard-and-Reports-family screen uses) — reusing the identical custom-date-range form pattern and `Table`/`EmptyState` components Monthly Summary and Investment already use, not a new design. No PDF/Excel export was added for this screen — FR-RPT-06/07 name only the Monthly Summary and general entries as exportable, not this report specifically. This closes FR-PINC-08's own "arbitrary range" gap at the same time, since both requirements describe the same underlying capability from two SRS sections (§3.3 and §3.13).
...
- FR-RPT-05 and FR-AUD-06 are now both fully `Implemented` (§13/§14) — no Phase 5 requirement remains disclosed as a gap or a partial limitation; only FR-WARN-05 stays deferred, per an explicit approved decision, not an oversight.
- The Phase 5 Playwright suite has zero conditional/data-dependent skips — the one remaining conditional skip (asset history, closure §13's sibling fix) was replaced with the test creating its own Asset fixture directly.

## Related SRS Requirements

FR-RES-01 to 11, FR-WARN-01 to 04 (FR-WARN-05 deferred), FR-RPT-01 to 09, FR-AUD-04 to 06, FR-PINC-08, BR-01, BR-06, BR-07, BR-10, DR-07, DR-08, DR-09, NFR-PERF-04, NFR-PERF-05, NFR-PERF-06, NFR-SEC-04, AC-02 (resolved per above), AC-08, AC-09, AC-10, UC-10, UC-12, UC-13, UC-14, UC-18, UC-21.

## Approval Status

Approved by the user (all listed decisions, including the AT WASTE resolution direction, the exact dependency versions, and the efficiency/reporting constraints) before implementation began; implemented in Phase 5 per this record. The BR-07/FR-RES-06 visibility fix (§12) was a correctness fix made during implementation to satisfy an already-approved, unambiguous CLAUDE.md rule — not a new decision requiring separate approval. The Phase 5 closure pass (§13/§14: FR-AUD-06's live-lookup fix, FR-RPT-05's build-out, and the Playwright skip removal) was explicitly requested by the user as a condition of closing out Phase 5, and is recorded here rather than as a separate ADR since it completes decisions this document already covers.
