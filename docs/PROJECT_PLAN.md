...
**Two disclosed gaps, both approved-scope, neither silently dropped:** FR-OFF-12 (mark figures provisional while offline uploads are pending) was not built this phase — the Phase 5 Dashboard/Monthly Summary screens do not yet mark totals provisional; the Sync Center's own pending count is the only place this is currently visible. NFR-SEC-09 (clear offline device data on sign-out once nothing is pending) is partial — per-user isolation is enforced and nothing is ever silently erased while work is pending, but automatic clearing once the queue is empty is not implemented. See `docs/REQUIREMENTS_TRACEABILITY.md` for the row-by-row detail.
