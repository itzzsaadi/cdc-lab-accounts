...
| FR-PINC-08 | Show total per party and combined party income for any range | M | Phase 3B / Phase 4 | `src/server/queries/party-income.ts` | `tests/integration/queries/party-income-grid.test.ts`, `tests/integration/queries/party-monthly-totals.test.ts` | Test | Partial | Per-party and grand totals are shown for the selected month; an arbitrary custom date range (vs. a whole calendar month) is still not a selectable option — deferred, likely Phase 5's date-range work. |
...
| FR-AUD-06 | (Should) Highlight changes to entries dated more than one month in the past | S | Phase 5 | `src/server/queries/audit-log.ts` (`isOverOneMonthOld`) | `tests/integration/queries/audit-log.test.ts` | Test | Implemented | Whole-month-granularity rule against the entity's own business date (`expenseDate`/`incomeDate`/`periodMonth`), read from the audit row's own JSON values — **known limitation:** `capital_contribution` and `asset` audit rows carry no such date field yet, so this flag is `null` (shown as "—") for those two entity types, not silently `false`. |
...
| FR-RPT-05 | Show income by party across a chosen range | M | Phase 5 | — | — | Test | Not Started | **Gap, disclosed:** no dedicated "income by party, arbitrary range" report screen was built in Phase 5. Phase 4's `getPartyMonthlyTotals` covers the Monthly Party Bill screen's own per-party/per-month total, but not a chosen-range report. Deferred; revisit alongside FR-PINC-08's same "arbitrary range" gap (ADR-0006). |
...
| AC-08 | Operator account shown to have no route to profit/loss/investment, incl. direct server request | N/A | Phase 5 | `src/lib/permissions/matrix.ts` | `tests/integration/authorization/phase5-authorization-sweep.test.ts`, `tests/e2e/phase5-reporting.spec.ts`, `tests/e2e/monthly-workflows.spec.ts` | Demonstration | Implemented | Closed out in Phase 5 as planned — every financial query/route (Monthly Summary, Dashboard, Audit Log, exports, Investment) proven denied to a direct `OPERATOR` call/request, not merely hidden navigation. |
