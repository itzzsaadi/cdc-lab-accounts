...
Every financial entry table carries `is_archived`, **except `assets`, which uses `status ENUM ('ACTIVE', 'ARCHIVED')` as its sole archive representation** — matching SRS §6's literal per-table schema (see `docs/adr/0002-phase-1-schema-clarifications.md` decision 6). Never add `is_archived` to `assets` alongside `status`; one archive representation per table, not two. "Delete" in the UI always means archive (set `is_archived = true`, or for assets, `status = 'ARCHIVED'`), never a SQL `DELETE` on a financial record (CON-04, DR-04, BR-15) — enforced at the database level from Phase 1 onward by a `BEFORE DELETE` rejection trigger on every business, financial, master-data, settings, user, and audit table, not merely by the absence of delete code. Archiving is reversible in principle and always preserves history. This applies to daily expenses, monthly expenses, party income, counter income, capital contributions, and assets. Master data (parties, items, categories, vendors) and `users` use `is_active` (the inverse boolean sense — true means usable, not true means removed) rather than `is_archived`; this naming difference is deliberate, matching SRS §6's actual column names, not an inconsistency to "fix." Master data is also archived, never deleted, so historical entries referencing them remain intact (FR-MST-05, DR-06) — though per ADR-0002 decision 8, a rename does change the *display* name shown on historical entries; only the figures and referential links are protected, not frozen historical wording.
